...
extern "C" __global__ void lookupFlatten(SpecularPath* specularPaths, uint32_t *specularPathCounter, const FirstDiffuse* firstDiffuses, int2 imageSize, PathCache pathCache )
{
    int xi = threadIdx.x + blockDim.x * blockIdx.x;
    int yi = threadIdx.y + blockDim.y * blockIdx.y;
    ...
    float3 p = firstDiffuse.p;
    int nPaths = 0;
    pathCache.lookUpIndex(p, [&](int index) {
        nPaths++;
    });
